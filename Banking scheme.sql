DROP DATABASE IF EXISTS Bank_scheme;
CREATE DATABASE IF NOT EXISTS Bank_scheme;
USE Bank_scheme;
...
ALTER TABLE ACCOUNT
    ...
    MODIFY LAST_ACTIVITY_DATE DATE;

ALTER TABLE ACC_TRANSACTION
    MODIFY FUNDS_AVAIL_DATE DATE,
    MODIFY TXN_DATE DATE;

ALTER TABLE INDIVIDUAL
    MODIFY BIRTH_DATE DATE;

ALTER TABLE BUSINESS
    MODIFY INCORP_DATE DATE;
...
ALTER TABLE PRODUCT
    MODIFY DATE_OFFERED DATE,
    MODIFY DATE_RETIRED DATE;


SELECT I.FIRST_NAME, I.LAST_NAME, C.CITY FROM CUSTOMER C JOIN INDIVIDUAL I ON C.CUST_ID = I.CUST_ID WHERE C.CITY LIKE 'W%'
UNION ALL
SELECT O.FIRST_NAME, O.LAST_NAME, C.CITY FROM CUSTOMER C JOIN OFFICER O ON C.CUST_ID = O.CUST_ID WHERE C.CITY LIKE 'W%';

UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 1, TELLER_EMP_ID = 6
WHERE TXN_ID = 1;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 1, TELLER_EMP_ID = 7
WHERE TXN_ID = 2;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 1, TELLER_EMP_ID = 8
WHERE TXN_ID = 3;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 1, TELLER_EMP_ID = 9
WHERE TXN_ID = 4;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 2, TELLER_EMP_ID = 11
WHERE TXN_ID = 5;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 2, TELLER_EMP_ID = 12
WHERE TXN_ID = 6;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 3, TELLER_EMP_ID = 14
WHERE TXN_ID = 7;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 3, TELLER_EMP_ID = 15
WHERE TXN_ID = 8;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 4, TELLER_EMP_ID = 17
WHERE TXN_ID = 9;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 4, TELLER_EMP_ID = 18
WHERE TXN_ID = 10;
UPDATE ACC_TRANSACTION
SET EXECUTION_BRANCH_ID = 4, TELLER_EMP_ID = 18
WHERE TXN_ID > 10;

SELECT a.account_id AS account_number, a.avail_balance AS available_balance, a.open_date AS opening_date,
CONCAT(e.First_Name, ' ', e.Last_Name) AS employee_name FROM account a JOIN customer c ON a.cust_id = c.cust_id JOIN employee e ON a.open_emp_id = e.emp_id WHERE e.First_Name = 'Paula' AND e.Last_Name = 'Roberts' ORDER BY a.avail_balance ASC;
